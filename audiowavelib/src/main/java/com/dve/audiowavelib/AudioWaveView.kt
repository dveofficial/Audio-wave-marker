...
class AudioWaveView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    private val mSelectedLinePaint: Paint = Paint()
    private val mUnselectedLinePaint: Paint = Paint()
    private val mUnselectedBkgndLinePaint: Paint = Paint()
    private val mBorderLinePaint: Paint = Paint()
    private val mPlaybackLinePaint: Paint = Paint()
    private val mTimecodePaint: Paint = Paint()
    ...
    private var mPlaybackPos = 0
    private var mDensity = 0f
    private var mInitialScaleSpan = 0f
    private val mListener: WaveformListener? = null
    ...
    init {
        ...
        mGridPaint.color = ContextCompat.getColor(context, R.color.grid_line)
        mSelectedLinePaint.isAntiAlias = false
        mSelectedLinePaint.color = ContextCompat.getColor(context, R.color.waveform_selected)
        mUnselectedLinePaint.isAntiAlias = false
        mUnselectedLinePaint.color = ContextCompat.getColor(context, R.color.waveform_unselected)
        mUnselectedBkgndLinePaint.isAntiAlias = false
        mUnselectedBkgndLinePaint.color = ContextCompat.getColor(context, R.color.waveform_unselected_bkgnd_overlay)
        ...
        mBorderLinePaint.color = ContextCompat.getColor(context, R.color.selection_border)
        mPlaybackLinePaint.isAntiAlias = false
        mPlaybackLinePaint.color = ContextCompat.getColor(context, R.color.playback_indicator)
        mTimecodePaint.textSize = 12f
        mTimecodePaint.isAntiAlias = true
        mTimecodePaint.color = ContextCompat.getColor(context, R.color.timecode)
        mTimecodePaint.setShadowLayer(2f, 1f, 1f, ContextCompat.getColor(context, R.color.timecode_shadow))
    ...
    fun setSoundFile(soundFile: SoundFile) {
        ...
        mSoundFile?.let {
            ...
            mSamplesPerFrame = it.samplesPerFrame
        }
    ...
    private fun computeDoublesForAllZoomLevels() {
        ...
        mValuesByZoomLevel = arrayOf(DoubleArray(5))
        ...
        for (j in 2..4) {
            ...
            for (i in 0 until mLenByZoomLevel!![j]) {
                mValuesByZoomLevel!![j][i] = 0.5 * (mValuesByZoomLevel!!.get(j - 1)[2 * i] +
                        mValuesByZoomLevel!!.get(j - 1)[2 * i + 1])
            }
        }
        mZoomLevel = when {
            numFrames > 5000 -> {
    ...
    fun drawWaveformLine(canvas: Canvas, x: Float, y0: Float, y1: Float, paint: Paint) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        i = 0
        while (i < width) {
            var paint: Paint = if (i + start in mSelectionStart until mSelectionEnd) {
                mSelectedLinePaint
            } else {
                drawWaveformLine(canvas, i.toFloat(), 0f, measuredHeight.toFloat(),
                        mUnselectedBkgndLinePaint)
                mUnselectedLinePaint
            }
            drawWaveformLine(
                    canvas, i.toFloat(),
                    (ctr - mHeightsAtThisZoomLevel!![start + i]).toFloat(),
                    (ctr + 1 + mHeightsAtThisZoomLevel!![start + i]).toFloat(),
                    paint)
        ...
        while (i < width) {
            ...
            if (integerTimecodeNew != integerTimecode) {
                ...
                canvas.drawText(timecodeStr, i - offset, (12 * mDensity), mTimecodePaint)
            }
        }
